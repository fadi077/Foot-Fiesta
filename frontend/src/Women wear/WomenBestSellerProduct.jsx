...
function WomenBestSellerProducts() {
  ...
  return (
    <div className="text-center bg-[#c4b4b4] text-gray-800 py-8">
      {cartItem && (
          ...
          <div className="popup-content bg-white p-6 rounded-lg flex flex-col items-center gap-4 shadow-lg max-w-sm w-full relative">
            ...
            <div className="flex gap-4">
              <button className="btn bg-gray-500 text-white hover:bg-gray-400 px-4 py-2 rounded-md"
                onClick={() => { navigate('/women'); setCartItem(null) }}>
                Continue Shopping
              </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {shoes.map((product, index) => (
          <div key={index} className="flex flex-col items-center p-4 glassy-card rounded-lg shadow-lg transition duration-300 ease-in-out transform hover:scale-105 hover:bg-pink-100">
...
export default WomenBestSellerProducts;
...
const styles = `
  ...
  .glassy-card:hover {
    ...
    background-color: rgba(255, 182, 193, 0.3); /* Light pink hover effect */
  }
